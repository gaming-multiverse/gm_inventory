...
export const ItemNotificationsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useNuiEvent<[item: SlotWithItem, text: string, count?: number]>('itemNotify', ([item, text, count]) => {
    // Display the removed or added amount based on 'count', not 'item.count'
    const displayCount = count || 0; // This should be the amount that was added/removed
    const itemCount = item.name == "money"
      ? displayCount.toFixed(2)
      : displayCount.toLocaleString('en-us');

    add({
      item: item,
      text: displayCount ? `${Locale[text]} ${itemCount}x` : `${Locale[text]}`
    });
  });
