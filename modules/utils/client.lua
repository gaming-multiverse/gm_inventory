...
function Utils.CreateBlip(settings, coords)
	...
	if IS_RDR3 then
		...
		SetBlipSprite(blip, GetHashKey(settings.id), true)

		local blipColor = GetHashKey(settings.colour)
		BlipAddModifier(blip, blipColor)

		local cleanName = string.gsub(settings.name, "^ox_shop_", "")
		SetBlipName(blip, cleanName)
	end
